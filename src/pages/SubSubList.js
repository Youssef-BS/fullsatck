...
const SubSubList = () => {
  ...
  const pCatStat = useSelector((state) => state.pSubSubCategory.pSubSubCategories);

  
  const data1 = pCatStat.map((category, index) => ({
    key: index + 1,
    name: category.name,
    category: category.Subcategory ? category.Subcategory.name : 'No Subcategory',  
    action: (
      <>
        <Link
  ...
  return (
    <div>
      <h3 className="mb-4 title">Product SubSubCategories</h3>
      <div>
