...
function ProductDetail() {
  ...
  if (ProductState?.extra_image) {
    try {
      Extra_images = JSON.parse(ProductState.extra_image);
    } catch (e) {
      if (typeof ProductState.extra_image === 'string') {
        Extra_images = ProductState.extra_image.split(',');
      } else {
        console.error('Unexpected format for extra_image:', ProductState.extra_image);
      }
    }
  }
    ...
    <div id="mainbody" className="mainbody pinfo">
      ...
      <div className="availability-container w-100 d-flex">
        <div className="availability align-self-center d-flex">
          <span className="product-box__availability--in-stock">{product.availability}</span>
        </div>
      </div>
                    ...
                    <div className="accordion accordion--products-details" id="detailsAccordion">
                      <div className="card">
                        <div className="card-header" id="heading-details-1">
                          <div
                            className={`collapsed ${activeSection === 'description' ? 'show' : ''}`}
                            type="button"
                            data-toggle="collapse"
                            data-target="#details-1"
                            aria-expanded={activeSection === 'description'}
                        ...
                        <div id="details-1" className={`collapse ${activeSection === 'description' ? 'show' : ''}`} aria-labelledby="heading-details-1" data-parent="#detailsAccordion">
                          <div className="card-body">
                            <p>{ProductState.description || 'Loading...'}</p>
                          </div>
                        </div>
                      </div>
                      <div className="card">
                        <div className="card-header" id="heading-details-2">
                          <div
                            className={`collapsed ${activeSection === 'light-source' ? 'show' : ''}`}
                            type="button"
                            data-toggle="collapse"
                            data-target="#details-2"
                            aria-expanded={activeSection === 'light-source'}
                            aria-controls="details-2"
                            onClick={() => handleSectionClick('light-source')}
                          >
                            Light Source / Optics
                          </div>
                        </div>
                        <div id="details-2" className={`collapse ${activeSection === 'light-source' ? 'show' : ''}`} aria-labelledby="heading-details-2" data-parent="#detailsAccordion">
                          <div className="card-body">
                            <p>{sectionData['light-source'] || 'Loading...'}</p>
                          </div>
                        </div>
                      </div>
                      <div className="card">
                        <div className="card-header" id="heading-details-3">
                          <div
                            className={`collapsed ${activeSection === 'another-section' ? 'show' : ''}`}
                            type="button"
                            data-toggle="collapse"
                            data-target="#details-3"
                            aria-expanded={activeSection === 'another-section'}
                            aria-controls="details-3"
                            onClick={() => handleSectionClick('another-section')}
                          >
                            Another Section
                          </div>
                        </div>
                        <div id="details-3" className={`collapse ${activeSection === 'another-section' ? 'show' : ''}`} aria-labelledby="heading-details-3" data-parent="#detailsAccordion">
                          <div className="card-body">
                            <p>{sectionData['another-section'] || 'Loading...'}</p>
                          </div>
                        </div>
                      </div>
